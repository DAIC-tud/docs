...
* The [hard limits](../../../docs/job_submissions/#partitions-and-quality-of-service) are in place only to protect the cluster from extreme overloads. The guiding principles of the cluster are fair-share and fair-use: all users should be able to use the cluster at the same time, and nobody should cause problems for the cluster or other users (see [Slurm's job scheduling and waiting times](../../../docs/job_submissions/#slurms-job-scheduling-and-waiting-times)). So you need to make sure that your jobs do not unfairly hinder other jobs.
...
* See [Jobs on GPU resources](../../../docs/job_submissions/#jobs-on-gpu-resources)
...
* There is also a per-user and per-QoS memory limit for the combined requested memory of all running jobs (of a user) in a certain QoS (see  [Partitions and Quality of service](../../../docs/job_submissions/#partitions-and-quality-of-service)). So, to run the most jobs at the same time, don’t request more memory than your jobs need.
